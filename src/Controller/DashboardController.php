...
class DashboardController extends AppController
{
    public function index()
    {
        ...
        $sWhere[date('Ym')] = $cur_month;
        $sWhere_customer[date('Ym')] = $cur_month_customer;
        ...
        $userBusinessSignedMonth = $this->Businesses->find()
            ...
            ->group(['userid'])
            ->where($sWhere + ['Businesses.state' => 2])
            ->contain(['Users'])
            ->limit(null);
        ...
        $customerData = $this->Customers->find()
            ->select(['month' => 'MONTH(created)','total' => 'count(id)'])
            ->where(['created >=' => new Date('-6 months')])
            ->group(['month'])
            ->order('month ASC');
        ...
        $businessSingedOrClosedData = $this->Businesses->find()
            ...
            ->where(['modified >=' => new Date('-6 months')])
            ->group(['month'])
            ->order('month ASC');
    ...
    public function sendReporter(){
        ...
        $reporterUrl = Router::url(['controller' => 'Dashboard','action' => 'draw'], true);
        ...
        <div>查看日報鏈接'.Router::url(['controller' => 'Dashboard','action' => 'index'],true).'</div>
    ...
    public function draw()
    {
        ...
        $sWhere["DATE_FORMAT(Businesses.created,'%Y%m')"] = date('Ym');
        $sWhere_customer["DATE_FORMAT(Customers.created,'%Y%m')"] = date('Ym');
