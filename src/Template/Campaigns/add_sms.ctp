...
<?= $this->start('top_nav') ?>
    <span class="navbar-brand">发送短信</span>
</div>
<div class="collapse navbar-collapse" id="example-navbar-collapse">
    <ul class="nav navbar-nav">
        <li><?= $this->Html->link(__('群發推廣列表'), ['action' => 'index']) ?></li>
        <li><?= $this->Html->link(__('群发邮件'), ['action' => 'add-email']) ?></li>
        <li><?= $this->Html->link(__('群发彩信'), ['action' => 'add-mms']) ?></li>
<?= $this->end() ?>

<div class="campaigns columns content">
    <div class="ui top attached tabular menu">
      <a class="<?php if (!isset($type) || $type== 'normal'): ?>active<?php endif ?> item" data-tab="normal">短信群发</a>
      <a class="<?php if (isset($type) && $type== 'test'): ?>active<?php endif ?> item" data-tab="test">短信测试</a>
    </div>
    <div class="ui bottom attached active tab segment" data-tab="normal">
        ...
        <fieldset>
            ...
            <?php
                echo $this->Form->control('content', ['label' => '內容', 'required' => true, 'class' => 'editor', 'type' => 'textarea']);
            ?>
        </fieldset>
    ...
    <div class="ui bottom attached tab segment" data-tab="test">
        ...
        <fieldset>
            ...
            <?php
                echo $this->Form->control('code', ['type' => 'number','label' => '區號','default' => 852, 'required' =>true]);
                echo $this->Form->control('number', ['type' => 'number','label' => '收件人', 'required' =>true]);
                echo $this->Form->control('content', ['label' => '內容', 'required' => true, 'class' => 'editor', 'type' => 'textarea', 'required' =>true]);
            ?>
        </fieldset>
        <?= $this->Form->button(__('提交'),['class' => ['btn','btn-primary'], 'id' => 'submit']) ?>
        <?= $this->Form->end() ?>
